...
-- === UP REVISION (升级操作) === --
-- 执行命令: psql -U admin -d stockdb -c "\i migrations/20251215_000000_complete_schema.sql"
-- 或在脚本开头添加: -- MIGRATE_UP
...
-- === DOWN REVISION (回滚操作) === --
-- 执行命令: psql -U admin -d stockdb -c "\i migrations/20251215_000000_complete_schema.sql"
-- 或在脚本开头添加: -- MIGRATE_DOWN
--
-- 注意: 回滚操作将删除所有表和数据，谨慎执行!
-- 回滚顺序: 从依赖最多的表开始，依次删除到依赖最少的表

-- 以下是回滚操作的SQL代码，使用时需要取消注释
/*
-- 1. 删除索引（顺序无关紧要）
...
DROP INDEX IF EXISTS idx_stocks_stock_code;
DROP INDEX IF EXISTS idx_stocks_market_type;
...
DROP INDEX IF EXISTS idx_favorites_stock_id;
DROP INDEX IF EXISTS idx_favorites_added_at;

DROP INDEX IF EXISTS idx_friend_relationships_user_id;
DROP INDEX IF EXISTS idx_friend_relationships_friend_id;
DROP INDEX IF EXISTS idx_friend_relationships_status;
DROP INDEX IF EXISTS idx_friend_relationships_created_at;

DROP INDEX IF EXISTS idx_friend_messages_sender_id;
DROP INDEX IF EXISTS idx_friend_messages_receiver_id;
DROP INDEX IF EXISTS idx_friend_messages_sent_at;
DROP INDEX IF EXISTS idx_friend_messages_message_type;
DROP INDEX IF EXISTS idx_friend_messages_is_read;

DROP INDEX IF EXISTS idx_group_messages_group_id;
DROP INDEX IF EXISTS idx_group_messages_sender_id;
DROP INDEX IF EXISTS idx_group_messages_sent_at;
DROP INDEX IF EXISTS idx_group_messages_message_type;

DROP INDEX IF EXISTS idx_ai_recommendations_user_id;
DROP INDEX IF EXISTS idx_ai_recommendations_stock_id;
DROP INDEX IF EXISTS idx_ai_recommendations_recommended_at;
DROP INDEX IF EXISTS idx_ai_recommendations_confidence_score;
...
DROP INDEX IF EXISTS idx_market_data_price;
...
DROP INDEX IF EXISTS idx_user_interests_interest_type;
DROP INDEX IF EXISTS idx_user_interests_interest_value;

DROP INDEX IF EXISTS idx_system_notifications_user_id;
DROP INDEX IF EXISTS idx_system_notifications_notification_type;
...
DROP INDEX IF EXISTS idx_stock_comments_stock_id;
DROP INDEX IF EXISTS idx_stock_comments_user_id;
DROP INDEX IF EXISTS idx_stock_comments_parent_id;
DROP INDEX IF EXISTS idx_stock_comments_created_at;
DROP INDEX IF EXISTS idx_stock_comments_is_deleted;
...
-- 3. 删除迁移历史记录
DELETE FROM migration_history WHERE version = '20251215_000000';

SELECT '✅ 完整数据库迁移回滚完成!' as migration_result;
*/

-- === MIGRATION SCRIPT INFO === --
-- 脚本版本: 1.0.0
-- 支持的PostgreSQL版本: 12.0+
-- 作者: 智能鑫AI系统开发团队
-- 生成时间: 2025-12-15 00:00:00
